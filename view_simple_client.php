...
for($i = 0; $i < sizeof($tickets); $i++){
  if($tickets->supportticket[$i]->tid == $tickid){
    ...
    // $userlabel = $tickets->supportticket[$i]->messages->message['id'];
    // $ticketusern = $users->user[$i]->username; 
    //   foreach ($tickets->supportticket[$i]->messages->children() as $mes)
    // {
    //    $mesuser = $tickets->supportticket[$i]->messages->message['id'];
    // }
...
?><html>
  ...
  <body>
    ...
    <h1>Details</h1>
    <div class="linkback">
      <a href="./tickets_simple_client.php" id="btn_back" class="back">Back</a>
    </div>
    <div>
