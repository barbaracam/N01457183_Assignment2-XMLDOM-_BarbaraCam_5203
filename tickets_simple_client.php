...
for($i = 0; $i < sizeof($doc); $i++){
    if($doc->supportticket[$i]->client == $userid){
        $rows .= '<tr>';
        $rows .= '<td>'.$doc->supportticket[$i]->tid.'</td>';
        $rows .= '<td>'.$doc->supportticket[$i]->opendate.'</td>'; 
        $rows .= '<td>'.$doc->supportticket[$i]->status.'</td>';
        $rows .= '<td>'.$doc->supportticket[$i]->client.'</td>'; 
        $rows .= "<td><a href='./view_simple_client.php?tid=".$doc->supportticket[$i]->tid."' class='bt' />View</a></td>"; 
    }
